...
public class LZW implements ICoders {
	...
	public byte[] encode(byte[] content) {
		...
		// --------------------------------------VARIÁVEIS-----------------------------------------------------//
		// ----------------------------------------------------------------------------------------------------//
		// define a hashmap e outras variaveis que serão utilizadas pelo
		// programa
		...
		while (true) {
			// Condição de saida: Simulando IOException
			if (j >= content.length) {
			...
			if (dictionary.containsKey(str + ch)) {
				...
				// Coloca os 12 bit sem um array e então esceve iusto no arquivo
				// output
	...
	public byte[] decode(byte[] content) {
		...
		int j = 0;
		int k = 0;
		// Recebe a primeira palavra no código e utiliza o caractere
		// correspodente
		buffer[0] = content[j++];
		buffer[1] = content[j++];
		...
		// A cada três bytes lidos gera os caracteres correspodentes
		try{
			while (true) {
				if (j >= content.length) {
					return saida;
